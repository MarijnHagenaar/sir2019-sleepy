...
class HomeController
{
    ...
    public function settings(Request $request, Response $response, $args)
    {
        ...
        if (filter_var($myIp, FILTER_VALIDATE_IP)) {
            ...
            echo self::exec('sudo cp -f /opt/processing/webserver/config.json.template /opt/processing/webserver/config.json && echo "OK (1/4)"');
            echo self::exec("sudo sed -i \"s/127.0.0.1/$myIp/\" /opt/processing/webserver/config.json && echo \"OK (2/4)\"");
...
sudo sshpass -p $robotPass scp $o -p \"\${files[@]}\" nao@$robotIp:/home/nao/cbsr/ && echo \"OK (2/4)\"");
    ...
    public function serviceLog(Request $request, Response $response, $args)
    {
        ...
        if (empty($service)) {
            ...
            echo self::exec("sudo journalctl -u $service -b --no-pager");
        }
    }
     ...
     * @throws \Exception
     */
    private static function exec($cmd, $timeout = 5)
    {
